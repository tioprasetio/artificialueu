...
function wishMe(){
    ...
    else if(hour>=12 && hour<17){ // Perbaiki kondisi untuk jam lebih dari 12
...
function takeCommand(message){
    if(message.includes('hai') || message.includes('halo')){
    ...
    else if(message.includes('apa itu') || message.includes('siapa itu') || message.includes('apa saja')) {
        window.open(`https://www.google.com/search?q=${message.replace(" ", "+")}`, "_blank");
        const finalText = "Ini yang saya temukan di internet mengenai " + message;
        speak(finalText);
    }
    else if(message.includes('wikipedia')) {
        window.open(`https://id.wikipedia.org/wiki/${message.replace("wikipedia", "")}`, "_blank");
        ...
        window.open(`https://www.google.com/search?q=${message.replace(" ", "+")}`, "_blank");
        const finalText = "Saya menemukan beberapa informasi mengenai " + message + " di Google";
        speak(finalText);
    }
}
